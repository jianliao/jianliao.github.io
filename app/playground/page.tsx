...
export default function Page() {
  ...
  const [capableResult, setCapableResult] = useState("no");
  const [summarizer, setSummarizer] = useState(null);
  const [tldr, setTldr] = useState(null);

  useEffect(() => {
    setMounted(true);
    // The stupidest way to make an async call in component intial renderring.
    startTransition(async () => {
      const capableResult = await capableToRunSummarizerService();
      setCapableResult(capableResult);

      const result = await getSummarizer({
        sharedContext:
          "This is a personal blog about python and machine learning.",
        type: "key-points",
        format: "markdown",
        length: "short",
      });
      setSummarizer(result);
    });
  }, []);
  ...
  return (
    <>
      <h1>Summarizer AI Status</h1>
      <h2>Supported: {isSummarizerServiceSupported() ? "Yes" : "No"}</h2>
      <h2>Capable: {capableResult}</h2>
      <p>
        {tldr === null ? (
          <button
            type="button"
            ...
            onClick={async () => {
              if (summarizer) {
                const summary = await (summarizer as any).summarize(blog);
                setTldr(summary);
              }
            }}
